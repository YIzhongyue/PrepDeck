...
def page_text(page: dict) -> tuple[str, list[str]]:
    ...
    texts = [b for b in page["blocks"] if b["kind"] in ("text", "table")]
    ...
    if not ocr and page.get("height"):
        ...
        if footer:
            selected = "\n".join(
                b["text"]
                for b in texts
                if b["id"] not in footer and b["method"] != "tesseract"
            )
...
def stream(
    document: dict, start: int, end: int
) -> tuple[str, list[tuple[int, int, list[str], int]]]:
    if (
        type(start) is not int
        or type(end) is not int
        or not 1 <= start <= end <= len(document["pages"])
    ):
...
def auto_sections(text: str, layout: dict) -> list[dict]:
    pattern = layout.get("sectionPattern")
    if not pattern:
        raise ValueError("This profile requires --sections or a sectionPattern")
    matches = list(re.finditer(pattern, text, re.M))
    sections, ids = [], set()
    import hashlib

    for i, match in enumerate(matches):
        # The exact normalized heading, not an exam-specific date parser,
        # determines scope. A repeated heading requires an explicit plan.
        label = " ".join(match.group(0).split())
        identity = "section-" + hashlib.sha256(label.encode()).hexdigest()[:12]
        if identity in ids:
            raise ValueError("Repeated section heading; supply --sections")
        ids.add(identity)
        stop = matches[i + 1].start() if i + 1 < len(matches) else len(text)
        if layout.get("endPattern"):
            end = re.search(layout["endPattern"], text[match.end() : stop], re.M)
            if end:
    ...
    if not sections:
        raise ValueError("No scoped headings found; supply --sections")
    return sections


def prepare(
    document: dict, layout_id: str, namespace: str, exam: dict, plan: dict | None = None
) -> dict:
    ...
    if document.get("version") != "1.0" or len(
        document.get("pages", [])
    ) != document.get("pageCount"):
        raise ValueError(
            "requires a complete extraction page inventory (including failed pages)"
        )
    text, ranges = stream(document, 1, document["pageCount"])
    if plan is None:
        if layout["sectionStrategy"] != "heading-pattern":
            raise ValueError(
                "This layout requires --sections: define question/answer scopes from bookmarks and original pages"
            )
        sections = auto_sections(text, layout)
    else:
    ...
    for section in sections:
        ...
        if plan is not None:
            chunk, local_ranges = stream(
                document, section.get("startPage"), section.get("endPage")
            )
        ...
        if role == "reference":
            warnings.append(
                f"{sid}: reference pages still need visual review for answer tables, examples and shared passages"
            )
            continue
        qpattern = re.compile(
            section.get("questionPattern", layout["questionPattern"]), re.M
        )
        apattern = re.compile(
            section.get("answerPattern", layout["answerPattern"]), re.M
        )
        if qpattern.groups < 1 or apattern.groups < 2:
            raise ValueError(
                "question patterns need a number group; answer patterns need number and label groups"
            )
        answers = (
        ...
        headers = [
            h
            for h in headers
            if not any(a.start() <= h.start() < a.end() for a in answers)
        ]
        boundaries = sorted(
            {
                right,
        ...
        for match in headers:
            ...
            if qid in by_id:
                raise ValueError(
                    f"Duplicate scoped question {qid}; split repeated examples/subquestions into distinct scopes"
                )
            ...
            candidate = {
                "type": "single_choice",
                "stem": body[: options[0].start()].strip() if options else body,
                "options": [],
                "correctAnswers": [],
                "tags": [namespace, sid],
            }
                ...
                candidate["options"].append(
                    {
                        ...
                        "text": body[
                            (
                                option.start(2)
                                if option.lastindex
                                and option.lastindex >= 2
                                and option.group(2) is not None
                                else option.end()
                            ) : finish
                        ].strip(),
                    }
            ...
            row = {
                ...
                "status": "review",
                "reason": "Check the complete question, option labels, figures and answer against original pages.",
                "data": candidate,
                "sources": {
                    "stem": refs,
                    "options": {o["id"]: refs for o in candidate["options"]},
                    "correctAnswers": {},
                },
                "reviewPages": pages,
            }
    ...
    for entry in entries:
        ...
        if row is None:
            entry["reason"] = (
                "No matching scoped question; inspect OCR and section boundaries."
            )
            continue
        ...
        if previous and previous != entry["correctAnswers"]:
            row["reason"] = (
                "Conflicting source answers; reconcile all answer entries before export."
            )
            ...
            row["data"]["type"] = (
                "multiple_choice"
                if len(entry["correctAnswers"]) > 1
                else "single_choice"
            )
            row["sources"]["correctAnswers"] = {
                label: entry["blockRefs"] for label in entry["correctAnswers"]
            }
    ...
    return {
        ...
        "sourceExpectations": [
            {"section": c["section"], "questionCount": c["expectedQuestions"]}
            for c in counts
            if c["expectedQuestions"] is not None
        ],
        "pageReviews": [],
        "questions": questions,
...
def main() -> int:
    ...
    parser.add_argument(
        "--profiles", type=Path, help="optional provider profile catalog"
    )
    ...
    parser.add_argument(
        "--sections",
        type=Path,
        help="reviewed page-range plan; required for scanned textbooks",
    )
    parser.add_argument("--output", "-o", type=Path, required=True)
    args = parser.parse_args()
    if args.output.resolve() in {
        p.resolve() for p in (args.document, args.sections) if p
    }:
    ...
    try:
        ...
        inventory = prepare(
            ...
            args.layout,
            args.namespace,
            {"id": args.exam_id, "name": args.exam_name, "language": args.language},
            plan,
        )
            ...
            json.dumps(
                {
                    **inventory["coverage"],
                    "status": "review",
                    "warnings": inventory["preparationReport"]["warnings"],
                },
                ensure_ascii=False,
            )
        )
