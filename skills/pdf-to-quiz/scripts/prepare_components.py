...
def prepare_components(document, profile_id, namespace, exam, plan=None):
    ...
    for row in base["questions"]:
        ...
        # Avoid duplicating hundreds of coarse evidence references in content.
        source_refs = list(
            {
                (s["page"], tuple(s["bbox"]) if s["bbox"] else None): s
...
def main():
    ...
    if a.output.resolve() in {
        v.resolve() for v in [a.document, a.profiles, a.sections] if v
    }:
        ...
        json.dumps(
            {
                **result["coverage"],
                "status": "review",
                "structuralIssues": len(result["report"]["componentIssues"]),
            }
        )
    )
    return 3
